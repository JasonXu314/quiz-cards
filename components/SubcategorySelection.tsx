...
const SubcategorySelection: React.FC<Props> = ({ dispatch, categories, subcategories }) => {
	...
	return (
		<div>
			...
			{categories.map((category, i) => <div key = {categoryTags[i]}>
				<h5>{category} {expanded[category] ? <ChevronUp height = "16" width = "16" onClick = {() => expandedDispatch(category)} /> : <ChevronDown height = "16" width = "16" onClick = {() => expandedDispatch(category)} />}</h5>
				{expanded[category] && <>
					{catToSubcat[category].map((subcategory: string, i: number) => <div key = {catToTags[category][i]}>
						<input onChange = {() => dispatch(subcategory)} type = "checkbox" id = {catToTags[category][i]} value = {subcategory}
